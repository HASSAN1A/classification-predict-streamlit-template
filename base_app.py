...
def main():
    ...
    st.subheader("If you can't reuse it, refuse it.")
    st.write("The climate is changing, so should we!")
    ...
    if selection == "Prediction":
        ...
        algo_option = ["Linear_Logistics", "SVC_Linear", "Multinomial_Naive_Bayes"]
        algo_selection = st.sidebar.selectbox("Choose Algorithm", algo_option)

        st.info("Predicting with " + algo_selection + " Model")

        # Selecting a Linear Logistic regression
        if algo_selection == "Linear_Logistics":
            pred = joblib.load(
                open(os.path.join("resources/Linear_Logistics_final_3.pkl"), "rb")
            )
            model_info = """
                Logistic regression is a supervised learning classification algorithm used
                to predict binary classification of a target variable.
                Logistic regression can, however, be used for multiclass classification as used in this project.
        ...
        if algo_selection == "SVC_Linear":
            pred = joblib.load(
                open(os.path.join("resources/SVC_linear_final_4.pkl"), "rb")
            )
            model_info = """
        ...
        # Creating the selection for the other algorithms
        if algo_selection == "Multinomial_Naive_Bayes":
            pred = joblib.load(
                open(os.path.join("resources/MultiNB_final_3.pkl"), "rb")
            )
            model_info = """
                Multinomial Naive Bayes algorithm is mostly used in Natural Language Processing (NLP).
        ...
        if st.button("Classify"):
            ...
            output = {
                0: "Neutral Tweet",
                1: "Supports Climate Change",
                2: "News-related Tweets",
            ...
            with col2:
                ...
                polarity_subjectivity = [
                    TextBlob(tweet_text).sentiment[0],
                    TextBlob(tweet_text).sentiment[1],
                ]
